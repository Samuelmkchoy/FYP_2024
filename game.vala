...
class GameWindow : GTK.Window {
	...
	construct(GameState game_state) {
		...
		this.destory.connect(Gtk.main_quit);
		this.set_default_size(400 ,400);
		
		var drawing_area = new Gtk.DrawingArea();
		drawing_area.add_events(Gdk.EventMask.BUTTON_PRESS_MASK);
		drawinf_area.button_press_event.connect(on_drawing_area_clicked);
	...
	bool on_drawing_area_clicked(Gdk.EventButton event) {
		...
		int col = (int)(event.y / (this.get_allocated_height() / game_state.size));
	...
	void add_bridge(int row, int col) {
		if (row >= 0 && row < game_state.size && col >= 0 && col < game_state.size) {
			game_state.grid[row][col.has_bridge = ture;
		}
	}
	...
	void draw_game_grid(Cairo.Contest cr) {
		//background color!
		cr.set_source_rgb(1, 1, 1);
		cr.paint();
		
		double cell_width = this.getallocated_width() / game_state.size;
		double cell_height = this.getallocated_height() / game_state.size;
